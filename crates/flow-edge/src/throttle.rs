...
impl Throttle {
    ...
    /// # Errors
    ///
    /// The remaining wait in µs when the client is still blocked.
    pub fn check(&self, client: IpAddr, now_us: u64) -> Result<(), u64> {
        match self.clients.get(&client) {
            Some(entry) if entry.blocked_until_us > now_us => Err(entry.blocked_until_us - now_us),
            _ => Ok(()),
        }
    }
    ...
    pub fn record_failure(&mut self, client: IpAddr, now_us: u64) -> u64 {
        ...
        let entry = self.clients.entry(client).or_insert(Entry {
            failures: 0,
            blocked_until_us: 0,
        });
...
mod tests {
    ...
    fn a_blocked_client_is_told_how_long_to_wait() {
        ...
        assert_eq!(throttle.check(client, NOW), Err(1_000_000));
        assert_eq!(throttle.check(client, NOW + 400_000), Err(600_000));
        assert!(
    ...
    fn a_legitimate_operator_is_never_shut_out_permanently() {
        ...
        assert_eq!(throttle.check(client, NOW), Err(MAX_DELAY_US));
