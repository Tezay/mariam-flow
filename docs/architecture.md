...
with no way for dashboard and API to disagree about their version.

Everything is served from one origin: the daemon answers `/api` and `/health`
and treats every other path as the dashboard's, so the session cookie is a
same-origin cookie and there is no CORS anywhere. Assets, the brand typeface
included, are self-hosted without exception: an appliance with no uplink is a
supported mode.

Its screens, the vocabulary its files are named with, and the rules each screen
applies are described in [dashboard.md](dashboard.md).
